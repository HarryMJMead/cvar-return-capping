...
class REINFORCE(BasePolicy):
    ...
    def preprocess_batch(self, tensordict_data: TensorDict):
        ...
        future_returns = torch.empty(time_steps, device=self.device, dtype=rewards.dtype)
        prev_future_return = 0
        for i in reversed(range(time_steps)):
            reward = rewards[i]
            done = dones[i]
            prev_future_return = future_returns[i] = reward + (~done)*prev_future_return

        tensordict_data['future_return'] = future_returns
    ...
    def optimize_model(self, sub_batch : TensorDict):
        ...
        offset_future_return = sub_batch['future_return'] - self.return_offset
        loss_value = - torch.mean(offset_future_return * log_prob)
