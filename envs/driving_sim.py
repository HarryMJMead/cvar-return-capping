...
class DrivingSim(core.Env):
    ...
    def __init__(self, seed=None, T=None, leader_probs=(.3,.3,.3,.0,.1), p_oil=0,
                 rewards_conf=None, nine_actions=False, reaction_delay=7, reward_normalisation=256):
        # General
        ...
        self.leader_probs = tuple(leader_probs)
        self.p_oil = p_oil
        self.oil_factor = 2
        self.l = 3.476
        self.lag = -2.5
    ...
    def reset(self, probs=None, leader_actions=None, L=None, init_state=None,
        ...
        return self.get_obs(), {"leader_probs" : self.leader_probs, "mask" : np.ones(self.action_space.n, dtype=np.int8)}
    ...
    def get_obs(self):
        ...
        return np.concatenate((self.agent_state, leader)).astype(np.float32) / 64
    ...
    def step(self, action):
        ...
        return self.get_obs(), tot_r / self.reward_normalisation, done, {"mask" : np.ones(self.action_space.n, dtype=np.int8)}
    ...
    def show_frame(self, i=None, ax=None, show_info=True, lab='agent',
        ...
        if show_info:
            ax.set_yticks([x, xl])
            ax.set_yticklabels([f'Δ={int(np.round(xl-x-self.l)):02d}m', ''],
