...
class SimpleGridEnv(Env):
    ...
    def __init__(self,     
        ...
        reward_normalisation: float = 1.0,
    ):
        """
    ...
    # def stoc_action(action:int):
    #     """
    #     Convert make selected action execute with 0.8 propability and slip to perpendicular direction with 0.1 probability each
    #     """
    ...
    def get_obs(self) -> np.array:
        state_index = self.to_s(*self.agent_xy)
        obs = np.zeros(self.nrow*self.ncol)
        obs[state_index] = 1
        return obs
    ...
    def create_agent_patch_img(self):
        """
        Create an image patch for the agent.

        @NOTE: If agent position is (x,y) then, to properly render it, we have to pass (y,x) as center to the AnnotationBbox.
        """
        # Load the image
        img = mpimg.imread('/Users/anyen/Documents/frozenlake-2hbt1/custom_gridworld/custom_gridworld/envs/img/images.jpeg')  # Update with the correct path to your image

        # Create an OffsetImage
        imagebox = OffsetImage(img, zoom=0.1)  # Adjust the zoom level as needed

        # Create an AnnotationBbox
        return AnnotationBbox(imagebox, (self.agent_xy[1]+.5, self.agent_xy[0]+.5), frameon=False, zorder=100)
    ...
    def render_initial_frame(self):
        ...
        # ax.add_artist(self.agent_patch)
        return None
