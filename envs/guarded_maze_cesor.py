...
class GuardedMazeCeSoR(gym.Env):
    ...
    def __init__(self, mode=1, rows=None, cols=None, action_noise=0.2, max_steps=None,
                 ...
                 reward_normalisation=256, deduction_limit=32, one_hot=True):
        self.mode = mode
        ...
        if self.continuous:
            ...
            if self.one_hot:
                    ...
                    shape=(self.rows*self.cols+1,), dtype=np.float32)
            else:
                self.observation_space = spaces.Box(
        ...
        self.state_xy, self.state = self._random_from_map(tuple(init_state))
    ...
    def one_hot_encoding(self):
        m = np.zeros((self.rows,self.rows), dtype=np.float32)  # i,j=0,...,n-1
    ...
    def reset(self, guard=None, guard_cost=None, init_state=None):
        ...
        obs = self.get_obs()
        return self.get_augmented_obs(obs), {"guard": self.curr_guard, "cost": self.curr_cost, "mask" : np.ones(self.action_space.n, dtype=np.int8)}
    ...
    def get_reward(self, done=False, moved=True, mid_cell=None):
        if self.detailed_r:
            ...
            r = 1*self.L if done else (
                -1 if self.nsteps<self.deduction_limit else 0)
    ...
    def step(self, action: int):
        ...
        self.tot_reward += r
        self.nsteps += 1
        info = {'path': self.is_long_path, "mask" : np.ones(self.action_space.n, dtype=np.int8)}

        if done:
            info = {'r': self.tot_reward, 'l': self.nsteps,
                    'path': self.is_long_path, "mask" : np.ones(self.action_space.n, dtype=np.int8)}

        return self.get_augmented_obs(obs), r / self.reward_normalisation, done, info
